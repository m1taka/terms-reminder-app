...
class GoogleCalendarService {
  ...
  async createEvent(reminder: Reminder): Promise<string | null> {
    ...
    try {
      ...
      if (reminder.type === 'court' || reminder.type === 'meeting') {
        (event as any).location = reminder.relatedCase || 'TBD';
      }
  ...
  async updateEvent(googleEventId: string, reminder: Reminder): Promise<string | null> {
  ...
  private formatDescription(reminder: Reminder): string {
  ...
  private calculateReminderMinutes(reminderDate: Date | string, dueDate: Date | string): number {
